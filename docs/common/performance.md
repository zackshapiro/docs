...
For queries run from iOS and Android, you can turn on query caching. See the [iOS](ios/queries.md#caching-queries) and [Android](android/queries.md#caching-queries) guides for more details. Caching queries will increase your mobile app's performance especially in cases where you want to display cached data while fetching the latest data from Parse.
